...
const partidoFutbol = torneo.programarPartido("P1", equipoFutbol1, equipoFutbol2, futbol);
if (partidoFutbol) {
    partidoFutbol.jugar(); // Valida con el método validar() de la clase Futbol
    console.log(`\n${partidoFutbol.toString()}`);
}
// Partido de básquet válido
const partidoBasquet = torneo.programarPartido("P2", equipoBasquet1, equipoBasquet2, basquet);
if (partidoBasquet) {
...
const partidoInvalidoFutbol = torneo.programarPartido("P3", equipoInvalidoFutbol, equipoFutbol2, futbol);
...
for (let i = 0; i < 6; i++) {
    equipoInvalidoBasquet.agregarJugador(new Jugador(`B${i}`, `Jugador B${i}`, 20));
}
const partidoInvalidoBasquet = torneo.programarPartido("P4", equipoInvalidoBasquet, equipoBasquet1, basquet);
